...
async function setupSimpleTokenEnvironment ({ qty = defaultQuantity } = {}) {
  ...
  const addresses = await eth.listAccounts()
  ...
  const token = await StandardToken.deploy(qty)
  assert.ok(token, 'should have published the token and returned a contract instance')
